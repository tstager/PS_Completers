...
function Get-Md5sumCompletionOptions {
    $cache = Get-Variable -Name 'Md5sumCompletionOptions' -Scope Script -ErrorAction SilentlyContinue
        ...
        return @(
            foreach ($option in Get-Md5sumCompletionOptions) {
                if ($option.StartsWith($currentWord, [System.StringComparison]::Ordinal)) {
                    New-Md5sumCompletionResult -CompletionText $option -ListItemText $option -ResultType 'ParameterName' -ToolTip 'Option for md5sum.'
                }
            }
        )
    }
