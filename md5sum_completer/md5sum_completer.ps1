...
function Get-Md5sumCompletionOptions {
    @('-b', '--binary', '-c', '--check', '--tag', '-t', '--text', '-z', '--zero', '--status', '--quiet', '--strict', '--ignore-missing', '--warn', '-h', '--help', '-V', '--version')
}
...
function Complete-Md5sum {
    ...
    $currentWord = if ($cursorPosition -gt $commandAst.Extent.EndOffset) {
        ...
        Get-Md5sumCurrentToken -Line $commandAst.ToString() -CursorPosition $cursorPosition -Fallback $wordToComplete
    }
