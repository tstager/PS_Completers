...
function Get-YesCompletionOptions {
    $cache = Get-Variable -Name 'YesCompletionOptions' -Scope Script -ErrorAction SilentlyContinue
...
function Complete-Yes {
    ...
    $currentWord = if ($cursorPosition -gt $commandAst.Extent.EndOffset) {
        ...
        Get-YesCurrentToken -Line $commandAst.ToString() -CursorPosition $cursorPosition -Fallback $wordToComplete
    }
        ...
        return @(
            foreach ($option in Get-YesCompletionOptions) {
                if ($option.StartsWith($currentWord, [System.StringComparison]::OrdinalIgnoreCase)) {
                    New-YesCompletionResult -CompletionText $option -ListItemText $option -ResultType 'ParameterName' -ToolTip 'Option for yes.'
                }
            }
        )
    }
