...
sha384sum_completer.ps1 registers a standalone native PowerShell completer for sha384sum and sha384sum.exe.

It is a help-driven completer with a static fallback for the sha384sum workflow. The script exposes the command's option catalog from the installed build, then falls back to filesystem path completion for operand slots.
...
```powershell
. .\sha384sum_completer\sha384sum_completer.ps1
```
...
- This completer is intentionally focused on the sha384sum workflow and the option set surfaced by the installed build.
