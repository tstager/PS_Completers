...
ln_completer.ps1 registers a standalone native PowerShell completer for ln and ln.exe.

It is a help-driven completer with a static fallback for the link creator workflow. The script exposes the command's option catalog from the installed build, then falls back to filesystem path completion for operand slots.
...
- This completer is intentionally focused on the link creator workflow and the option set surfaced by the installed build.
