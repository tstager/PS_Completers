...
function Remove-LinkOuterQuotes {
    param([string]$Value)
...
function Get-LinkPathCompletions {
    ...
    $cleanInput = Remove-LinkOuterQuotes -Value $InputPath
    ...
    foreach ($item in $items) {
        ...
        $quotedPath = ConvertTo-LinkQuotedValue -Value $pathText -AlwaysQuote $alwaysQuote
...
function Complete-Link {
    ...
    $currentWord = if ($cursorPosition -gt $commandAst.Extent.EndOffset) {
        ...
        Get-LinkCurrentToken -Line $commandAst.ToString() -CursorPosition ($cursorPosition - $commandAst.Extent.StartOffset) -Fallback $wordToComplete
    }
