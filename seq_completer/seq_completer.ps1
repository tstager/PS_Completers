...
function Get-SeqCompletionOptions {
    @(
    '-s', '--separator', '-t', '--terminator', '-w', '--equal-width', '-f', '--format', '-p', '-h', '--help', '-V', '--version'
    )
}
...
function Complete-Seq {
    ...
    $currentWord = if ($cursorPosition -gt $commandAst.Extent.EndOffset) {
        ...
        Get-SeqCurrentToken -Line $commandAst.ToString() -CursorPosition $cursorPosition -Fallback $wordToComplete
    }
