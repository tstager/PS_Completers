...
function Get-TouchCompletionOptions {
    $cache = Get-Variable -Name 'TouchCompletionOptions' -Scope Script -ErrorAction SilentlyContinue
...
function Complete-Touch {
    ...
    $currentWord = if ($cursorPosition -gt $commandAst.Extent.EndOffset) {
        ...
        Get-TouchCurrentToken -Line $commandAst.ToString() -CursorPosition $cursorPosition -Fallback $wordToComplete
    }
        ...
        return @(
            foreach ($option in Get-TouchCompletionOptions) {
                if ($option.StartsWith($currentWord, [System.StringComparison]::OrdinalIgnoreCase)) {
                    New-TouchCompletionResult -CompletionText $option -ListItemText $option -ResultType 'ParameterName' -ToolTip 'Option for touch.'
                }
            }
        )
    }
