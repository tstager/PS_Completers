...
function Get-StdbufCompletionOptions {
    $cache = Get-Variable -Name 'StdbufCompletionOptions' -Scope Script -ErrorAction SilentlyContinue
...
function Complete-Stdbuf {
    ...
    $currentWord = if ($cursorPosition -gt $commandAst.Extent.EndOffset) {
        ...
        Get-StdbufCurrentToken -Line $commandAst.ToString() -CursorPosition $cursorPosition -Fallback $wordToComplete
    }
        ...
        return @(
            foreach ($option in Get-StdbufCompletionOptions) {
                if ($option.StartsWith($currentWord, [System.StringComparison]::OrdinalIgnoreCase)) {
                    New-StdbufCompletionResult -CompletionText $option -ListItemText $option -ResultType 'ParameterName' -ToolTip 'Option for stdbuf.'
                }
            }
        )
    }
