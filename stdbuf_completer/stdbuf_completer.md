...
It is a help-driven completer with a static fallback for standard buffering control. The script exposes the command's option catalog and falls back to filesystem path completion for operand slots.
...
- `-i`, `-o`, `-e`, `--input`, `--output`, `--error`: `L`, `0`, `<size>`
...
- `--input` shows its documented values in both the separate and the attached form
