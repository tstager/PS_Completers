...
function Get-CksumCompletionOptions {
    $cache = Get-Variable -Name 'CksumCompletionOptions' -Scope Script -ErrorAction Ignore
    if ($null -ne $cache -and $null -ne $cache.Value) {
        return $cache.Value
    }

    $fallbackOptions = @('-a', '--algorithm', '--help', '--version')
    $commandCandidates = @('cksum.exe', 'cksum')
    foreach ($candidate in $commandCandidates) {
        ...
        if ($options.Count -gt 0) {
            Set-Variable -Name 'CksumCompletionOptions' -Value (@($options | Sort-Object)) -Scope Script
            Set-Variable -Name 'CksumCompletionDescriptions' -Value $descriptions -Scope Script
            return (Get-Variable -Name 'CksumCompletionOptions' -Scope Script).Value
        }
    }

    Set-Variable -Name 'CksumCompletionOptions' -Value $fallbackOptions -Scope Script
    return (Get-Variable -Name 'CksumCompletionOptions' -Scope Script).Value
}
...
function Get-CksumOptionDescription {
    param([string]$Option)

    $cache = Get-Variable -Name 'CksumCompletionDescriptions' -Scope Script -ErrorAction Ignore
        ...
        return @(
            foreach ($option in Get-CksumCompletionOptions) {
                if ($option.StartsWith($currentWord, [System.StringComparison]::Ordinal)) {
                    New-CksumCompletionResult -CompletionText $option -ListItemText $option -ResultType 'ParameterName' -ToolTip (Get-CksumOptionDescription -Option $option)
                }
            }
        )
    }
