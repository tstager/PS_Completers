...
basename_completer.ps1 registers a standalone native PowerShell completer for basename and basename.exe.

It is a help-driven completer with a static fallback for the basename path-splitting workflow. The script exposes a compact option catalog for common basename flags and falls back to filesystem path completion for operand slots.
...
```powershell
. .\basename_completer\basename_completer.ps1
```
...
- This completer is intentionally focused on the basename workflow and the option set surfaced in this repository's import-safe pattern.
