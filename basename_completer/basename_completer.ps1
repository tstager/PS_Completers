...
function Get-BasenameCompletionOptions {
    @(
    '-a', '--multiple', '-s', '--suffix', '-z', '--zero', '--help', '--version'
    )
}
...
function Complete-Basename {
    ...
    $currentWord = if ($cursorPosition -gt $commandAst.Extent.EndOffset) {
        ...
        Get-BasenameCurrentToken -Line $commandAst.ToString() -CursorPosition $cursorPosition -Fallback $wordToComplete
    }
        ...
        return @(
            foreach ($option in Get-BasenameCompletionOptions) {
                if ($option.StartsWith($currentWord, [System.StringComparison]::OrdinalIgnoreCase)) {
                    New-BasenameCompletionResult -CompletionText $option -ListItemText $option -ResultType 'ParameterName' -ToolTip 'Option for basename.'
                }
            }
        )
    }
