...
function Get-Base64CompletionOptions {
    $cache = Get-Variable -Name 'Base64CompletionOptions' -Scope Script -ErrorAction SilentlyContinue
...
function Complete-Base64 {
    ...
    $currentWord = if ($cursorPosition -gt $commandAst.Extent.EndOffset) {
        ...
        Get-Base64CurrentToken -Line $commandAst.ToString() -CursorPosition $cursorPosition -Fallback $wordToComplete
    }
        ...
        return @(
            foreach ($option in Get-Base64CompletionOptions) {
                if ($option.StartsWith($currentWord, [System.StringComparison]::OrdinalIgnoreCase)) {
                    New-Base64CompletionResult -CompletionText $option -ListItemText $option -ResultType 'ParameterName' -ToolTip 'Option for base64.'
                }
            }
        )
    }
