...
readlink_completer.ps1 registers a standalone native PowerShell completer for readlink and readlink.exe.

It is a help-driven completer with a static fallback for the resolver for symbolic links workflow. The script exposes the command's option catalog from the installed build, then falls back to filesystem path completion for operand slots.
...
```powershell
. .\readlink_completer\readlink_completer.ps1
```
...
- This completer is intentionally focused on the resolver for symbolic links workflow and the option set surfaced by the installed build.
