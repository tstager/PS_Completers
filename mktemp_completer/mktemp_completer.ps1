# mktemp tab completion for PowerShell
# Static option completion for mktemp.exe and mktemp.
...
function Get-MktempCompletionOptions {
    ...
    Set-Variable -Name 'MktempCompletionOptions' -Value $fallbackOptions -Scope Script
    return (Get-Variable -Name 'MktempCompletionOptions' -Scope Script).Value
}


function New-MktempCompletionResult {
    param(
...
function Get-MktempPathCompletions {
    ...
    foreach ($item in $items) {
        ...
        if ($item.PSIsContainer) {
            ...
            New-MktempCompletionResult -CompletionText $quotedPath -ListItemText $pathText -ResultType 'ProviderItem' -ToolTip $item.FullName
        }
    }
}
...
function Get-MktempOptionValueCompletions {
    ...
    $table['--suffix'] = @(
        @{ Text = '<suffix>'; Tip = 'Suffix appended to the template.' }
    )
    $table['-p'] = 'path'
    $table['--tmpdir'] = 'path'
    ...
    @(
        foreach ($entry in $values) {
            if ($entry.Text.StartsWith($prefix, [System.StringComparison]::Ordinal)) {
                New-MktempCompletionResult -CompletionText ($attached + $entry.Text) -ListItemText $entry.Text -ResultType 'ParameterValue' -ToolTip $entry.Tip
            }
        }
    )
}
