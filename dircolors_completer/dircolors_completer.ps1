...
function Get-DircolorsCompletionOptions {
    @(
    '-b', '--print-database', '-p', '--print-database', '-c', '--sh', '--help', '--version'
    )
}
...
function Complete-Dircolors {
    ...
    $currentWord = if ($cursorPosition -gt $commandAst.Extent.EndOffset) {
        ...
        Get-DircolorsCurrentToken -Line $commandAst.ToString() -CursorPosition $cursorPosition -Fallback $wordToComplete
    }
        ...
        return @(
            foreach ($option in Get-DircolorsCompletionOptions) {
                if ($option.StartsWith($currentWord, [System.StringComparison]::OrdinalIgnoreCase)) {
                    New-DircolorsCompletionResult -CompletionText $option -ListItemText $option -ResultType 'ParameterName' -ToolTip 'Option for dircolors.'
                }
            }
        )
    }
