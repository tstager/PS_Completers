...
function Get-UniqCompletionOptions {
    @(
    '-D', '--all-repeated', '-m', '--group', '-w', '--check-chars', '-c', '--count', '-i', '--ignore-case', '-d', '--repeated', '-s', '--skip-chars', '-f', '--skip-fields', '-u', '--unique', '-z', '--zero-terminated', '-h', '--help', '-V', '--version'
    )
}
...
function Complete-Uniq {
    ...
    $currentWord = if ($cursorPosition -gt $commandAst.Extent.EndOffset) {
        ...
        Get-UniqCurrentToken -Line $commandAst.ToString() -CursorPosition $cursorPosition -Fallback $wordToComplete
    }
