...
function Get-CsplitCompletionOptions {
    @(
    '-b', '--suffix-format', '-f', '--prefix', '-n', '--digits', '-s', '--quiet', '--silent', '-z', '--elide-empty-files', '-x', '--suppress-matched', '-k', '--keep-files', '--help', '--version'
    )
}
...
function Complete-Csplit {
    ...
    $currentWord = if ($cursorPosition -gt $commandAst.Extent.EndOffset) {
        ...
        Get-CsplitCurrentToken -Line $commandAst.ToString() -CursorPosition $cursorPosition -Fallback $wordToComplete
    }
