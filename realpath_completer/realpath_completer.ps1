...
function Get-RealpathCompletionOptions {
    @(
    '-q', '--quiet', '-s', '--strip', '--no-symlinks', '-z', '--zero', '-L', '--logical', '-P', '--physical', '-E', '--canonicalize', '-e', '--canonicalize-existing', '-m', '--canonicalize-missing', '--relative-to', '--relative-base', '-h', '--help', '-V', '--version'
    )
}
...
function Complete-Realpath {
    ...
    $currentWord = if ($cursorPosition -gt $commandAst.Extent.EndOffset) {
        ...
        Get-RealpathCurrentToken -Line $commandAst.ToString() -CursorPosition $cursorPosition -Fallback $wordToComplete
    }
        ...
        return @(
            foreach ($option in Get-RealpathCompletionOptions) {
                if ($option.StartsWith($currentWord, [System.StringComparison]::OrdinalIgnoreCase)) {
                    New-RealpathCompletionResult -CompletionText $option -ListItemText $option -ResultType 'ParameterName' -ToolTip 'Option for realpath.'
                }
            }
        )
    }
