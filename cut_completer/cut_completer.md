...
cut_completer.ps1 registers a standalone native PowerShell completer for cut and cut.exe.

It is a help-driven completer with a static fallback for the byte/character field extractor workflow. The script exposes the command's option catalog from the installed build, then falls back to filesystem path completion for operand slots.
...
```powershell
Register-ArgumentCompleter -Native -CommandName 'cut', 'cut.exe' -ScriptBlock {
    param($wordToComplete, $commandAst, $cursorPosition)

    Complete-Cut -wordToComplete $wordToComplete -commandAst $commandAst -cursorPosition $cursorPosition
}
```
...
```powershell
. .\cut_completer\cut_completer.ps1
```
...
- `-b`, `--bytes`, `-c`, `--characters`, `-f`, `--fields`: `<list>`
- `-d`, `--delimiter`: `<delim>`
...
- This completer is intentionally focused on the byte/character field extractor workflow and the option set surfaced by the installed build.
