...
shuf_completer.ps1 registers a standalone native PowerShell completer for shuf and shuf.exe.

It is a help-driven completer with a static fallback for the shuf workflow. The script exposes the command's option catalog from the installed build, then falls back to filesystem path completion for operand slots.
...
```powershell
. .\shuf_completer\shuf_completer.ps1
```
...
- `-i`, `--input-range`: `<lo-hi>`
- `-n`, `--head-count`: `<count>`
- `-o`, `--output`, `--random-source`: filesystem paths
...
- `--input-range` shows its documented values in both the separate and the attached form
- operand slots offer filesystem completion
- the completer remains importable through `Import-CompleterScript`

## Notes

- This completer is intentionally focused on the shuf workflow and the option set surfaced by the installed build.
