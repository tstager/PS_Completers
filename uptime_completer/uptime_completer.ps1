...
function Remove-UptimeOuterQuotes {
    param([string]$Value)
...
function Get-UptimePathCompletions {
    ...
    $cleanInput = Remove-UptimeOuterQuotes -Value $InputPath
    ...
    foreach ($item in $items) {
        ...
        $quotedPath = ConvertTo-UptimeQuotedValue -Value $pathText -AlwaysQuote $alwaysQuote
...
function Complete-Uptime {
    ...
    $currentWord = if ($cursorPosition -gt $commandAst.Extent.EndOffset) {
        ...
        Get-UptimeCurrentToken -Line $commandAst.ToString() -CursorPosition ($cursorPosition - $commandAst.Extent.StartOffset) -Fallback $wordToComplete
    }
