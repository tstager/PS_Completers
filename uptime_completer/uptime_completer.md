...
uptime_completer.ps1 registers a standalone native PowerShell completer for uptime and uptime.exe.

It is a help-driven completer with a static fallback for the uptime command surface. The script exposes the supported option catalog and falls back to filesystem path completion for operand slots.
...
```powershell
. .\uptime_completer\uptime_completer.ps1
```
...
- This completer is intentionally focused on the core uptime option surface and the standard operand fallback.
