...
function Get-TtyCompletionOptions {
    $cache = Get-Variable -Name 'TtyCompletionOptions' -Scope Script -ErrorAction SilentlyContinue
...
function Complete-Tty {
    ...
    $currentWord = if ($cursorPosition -gt $commandAst.Extent.EndOffset) {
        ...
        Get-TtyCurrentToken -Line $commandAst.ToString() -CursorPosition $cursorPosition -Fallback $wordToComplete
    }
        ...
        return @(
            foreach ($option in Get-TtyCompletionOptions) {
                if ($option.StartsWith($currentWord, [System.StringComparison]::OrdinalIgnoreCase)) {
                    New-TtyCompletionResult -CompletionText $option -ListItemText $option -ResultType 'ParameterName' -ToolTip 'Option for tty.'
                }
            }
        )
    }
