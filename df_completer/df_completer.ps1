...
function Get-DfCompletionOptions {
    @(
    '-a', '--all', '-B', '--block-size', '--total', '-h', '--human-readable', '-H', '--si', '-i', '--inodes', '-k', '-l', '--local', '--no-sync', '--output', '-P', '--portability', '--sync', '-t', '--type', '-T', '--print-type', '-w', '-x', '--exclude-type', '-V', '--version', '--help'
    )
}
...
function Complete-Df {
    ...
    $currentWord = if ($cursorPosition -gt $commandAst.Extent.EndOffset) {
        ...
        Get-DfCurrentToken -Line $commandAst.ToString() -CursorPosition $cursorPosition -Fallback $wordToComplete
    }
        ...
        return @(
            foreach ($option in Get-DfCompletionOptions) {
                if ($option.StartsWith($currentWord, [System.StringComparison]::OrdinalIgnoreCase)) {
                    New-DfCompletionResult -CompletionText $option -ListItemText $option -ResultType 'ParameterName' -ToolTip 'Option for df.'
                }
            }
        )
    }
